...
export function buildMotorCmd(motorType: 'seat' | 'head' | 'lumbar' | 'foot' | 'back', direction: number): Uint8Array {
  const cmdMap: Record<string, number[]> = {
    seat: CMD.MOTOR_SEAT,
    head: CMD.MOTOR_HEAD,
    lumbar: CMD.MOTOR_LUMBAR,
    foot: CMD.MOTOR_FOOT,
    back: CMD.MOTOR_BACK,
  };
  const cmd = cmdMap[motorType];
  // PRD: data packet byte-2 = motor type code
  return buildFrame(cmd, [0x00, cmd[1], 0x00, 0x00, direction & 0xFF]);
}

export function buildPositionCmd(preset: number): Uint8Array {
  return buildFrame(CMD.POSITION_PRESET, [0x00, 0x00, 0x00, 0x00, preset & 0xFF]);
}

export function buildMemoryRunCmd(slot: number): Uint8Array {
  return buildFrame(CMD.MEMORY_RUN, [0x00, 0x00, 0x00, 0x00, slot & 0xFF]);
}

export function buildMemorySetCmd(slot: number): Uint8Array {
  return buildFrame(CMD.MEMORY_SET, [0x00, 0x00, 0x00, 0x00, slot & 0xFF]);
}

export function buildMassageCmd(mode: number): Uint8Array {
  return buildFrame(CMD.MASSAGE, [0x00, 0x00, 0x00, 0x00, mode & 0xFF]);
}

export function buildMassageIntensityCmd(level: number): Uint8Array {
  return buildFrame(CMD.MASSAGE_INTENSITY, [0x00, 0x00, 0x00, 0x00, level & 0xFF]);
}

export function buildHeatingCmd(zone: 'seat' | 'back' | 'shoulder' | 'leg' | 'arm', level: number): Uint8Array {
  const cmdMap: Record<string, number[]> = {
    seat: CMD.HEATING_SEAT,
    back: CMD.HEATING_BACK,
    shoulder: CMD.HEATING_SHOULDER,
    leg: CMD.HEATING_LEG,
    arm: CMD.HEATING_ARM,
  };
  return buildFrame(cmdMap[zone], [0x00, 0x00, 0x00, 0x00, level & 0xFF]);
}

export function buildHeatingModeCmd(mode: number): Uint8Array {
  return buildFrame(CMD.HEATING_MODE, [0x00, 0x00, 0x00, 0x00, mode & 0xFF]);
}

export function buildVentilationCmd(zone: 'seat' | 'back', level: number): Uint8Array {
  const cmdMap: Record<string, number[]> = {
    seat: CMD.VENTILATION_SEAT,
    back: CMD.VENTILATION_BACK,
  };
  return buildFrame(cmdMap[zone], [0x00, 0x00, 0x00, 0x00, level & 0xFF]);
}

export function buildVentilationModeCmd(mode: number): Uint8Array {
  return buildFrame(CMD.VENTILATION_MODE, [0x00, 0x00, 0x00, 0x00, mode & 0xFF]);
}

export function buildTimerCmd(type: 'massage' | 'heating' | 'ventilation', minutes: number): Uint8Array {
  const cmdMap: Record<string, number[]> = {
    massage: CMD.MASSAGE_TIMER,
    heating: CMD.HEATING_TIMER,
    ventilation: CMD.VENTILATION_TIMER,
  };
  return buildFrame(cmdMap[type], [0x00, 0x00, 0x00, 0x00, minutes & 0xFF]);
}

export function buildLightCmd(mode: number): Uint8Array {
  return buildFrame(CMD.LIGHT, [0x00, 0x00, 0x00, 0x00, mode & 0xFF]);
}

export function buildLightColorCmd(r: number, g: number, b: number): Uint8Array {
  return buildFrame(CMD.LIGHT_COLOR, [0x00, r & 0xFF, g & 0xFF, b & 0xFF, 0x00]);
}

export function buildVibroCmd(mode: number): Uint8Array {
  return buildFrame(CMD.VIBRO, [0x00, 0x00, 0x00, 0x00, mode & 0xFF]);
}

export function buildVibroIntensityCmd(level: number): Uint8Array {
  return buildFrame(CMD.VIBRO_INTENSITY, [0x00, 0x00, 0x00, 0x00, level & 0xFF]);
}

export function buildAudioModeCmd(mode: number): Uint8Array {
  return buildFrame(CMD.AUDIO_MODE, [0x00, 0x00, 0x00, 0x00, mode & 0xFF]);
}

export function buildAudioTrebleCmd(value: number): Uint8Array {
  return buildFrame(CMD.AUDIO_TREBLE, [0x00, 0x00, 0x00, 0x00, value & 0xFF]);
}

export function buildAudioBassCmd(value: number): Uint8Array {
  return buildFrame(CMD.AUDIO_BASS, [0x00, 0x00, 0x00, 0x00, value & 0xFF]);
}

export function buildAudioVolumeCmd(value: number): Uint8Array {
  return buildFrame(CMD.AUDIO_VOLUME, [0x00, 0x00, 0x00, 0x00, value & 0xFF]);
}

export function buildChildLockCmd(lock: boolean): Uint8Array {
  return buildFrame(CMD.CHILD_LOCK, [0x00, 0x00, 0x00, 0x00, lock ? 0x01 : 0x00]);
}
